...
\begin{enumerate}
	...
	\item Произведение рядов. \\
	      \begin{mdframed}
		      ...
		      \textbf{Формальное произведение рядов:} \\
		      $\sum_{n=1}^{\infty} h_n$, где $h_n = \sum_{k=1}^{n} a_kb_{n-k+1}$
		      \[
			      h_1 = a_1b_1
		      \]
		      \[
		      ...
		      \]
		      \[
			      h_3 = a_1b_3 + a_2b_2 + a_3b_1
		      \]
		      \textbf{Теорема:} \\
		      Если ряды $\sum_{m=1}^{\infty} a_m$ и $\sum_{n=1}^{\infty} b_n$ сходятся абсолютно, \\
		      $\sum_{m=1}^{\infty} a_m = A; \sum_{n=1}^{\infty} b_n = B$, то \\ 
		      любое произведение рядов $\sum_{m=1}^{\infty} a_m \cdot \sum_{n=1}^{\infty} b_n$ сходится абсолютно к $A \cdot B$.
